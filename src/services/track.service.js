...
const ALLOWED_AUDIO_TYPES = ['audio/mp3','audio/flac' ,'audio/wav', 'audio/aac'];
...
const createTrack = async (userId, trackData, audioFile, coverFile) => {
  ...
  if (!ALLOWED_AUDIO_TYPES.includes(audioFile.mimetype))
    throw new BadRequestError("Invalid audio format. Only MP3, FLAC, WAV, and AAC are allowed.");
  ...
  let artworkUrl = "default-artwork.png";
  if (coverFile) {
    artworkUrl = await S3Utils.uploadToS3(coverFile, 'artwork');
  }
